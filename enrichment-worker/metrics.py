...
def _render_metrics() -> str:
    ...
    with _lock:
        ...
        for key, samples in sorted(_histograms.items()):
            ...
            # Build extra labels string (without the wrapping braces)
            if label_part:
                extra = "," + label_part[1:-1]  # strip leading { and trailing }
            else:
                extra = ""

            lines.append(f"# TYPE {name} summary")
            lines.append(f'{name}{{quantile="0.5"{extra}}} {p50:.4f}')
            lines.append(f'{name}{{quantile="0.9"{extra}}} {p90:.4f}')
            lines.append(f'{name}{{quantile="0.99"{extra}}} {p99:.4f}')
